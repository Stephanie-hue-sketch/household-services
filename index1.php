            ...
            <div class="row g-0 mx-lg-0">
                <div class="col-lg-12 ps-lg-0" style="min-height: 400px;">
                        ...
                        <div class="section-title text-start">
                            ...
                            <br>
                            <h2>List of Available Requests</h2>
                        </div>
<table id="printTable">
...
<th style="text-align: left;
 ...
 <th style="text-align: left;
  ...
  padding: 8px;">Status</th>
    <th style="text-align: left;
  padding: 8px;">Client Details</th>  
   <th style="text-align: left; padding: 8px;"></th>  
</tr>

<?php
$filter1 = $row1['Service_Type'];
$sql = "SELECT `requests`.`Request_ID`, `requests`.`Fee`, `requests`.`From_To`, `requests`.`Total_Fees`, `requests`.`Status`, `requests`.`Details`, `requests`.`Rooms`, `requests`.`Service_Type`, `users`.`Fullname`, `users`.`Location`, `users`.`Phone_Number`, `users`.`Email_Address` FROM `requests` JOIN `users` ON `requests`.`Client_ID` = `users`.`User_ID` WHERE `requests`.`Service_Provider_ID` = '$filter' OR `requests`.`Status` = 'Pending' OR `requests`.`Service_Type` = '$filter1'";
...
<td><?php echo($row["Fullname"]); ?> reach out on <?php echo($row["Phone_Number"]); ?> or <?php echo($row["Email_Address"]); ?> at <?php echo($row["Location"]); ?></td>
<?php
if($row['Status'] == 'Pending'){
?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to accept this request?')?window.location.href='insertion.inc.php?action=acceptR&id=<?php echo($row["Request_ID"]); ?>':true;" title='Accept Request'>Accept</button></td>
<?php
}else{
?>
<td></td>
