...
<head>
    <meta charset="utf-8">
    <title>Household Services System - Administrator Homepage</title>
...
$sql = "SELECT `User_ID`, `Fullname`, `Phone_Number`, `Email_Address`, `User_Type`, `Status` FROM `users`";
...
if($row['Status'] == 0){
?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to approve this user?')?window.location.href='insertion.inc.php?action=updateU&id=<?php echo($row["User_ID"]); ?>':true;" title='Approve User'>Approve</button></td>
<?php
}else{
?>
<td></td>
